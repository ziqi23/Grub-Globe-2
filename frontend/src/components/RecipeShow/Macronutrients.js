...
const Macronutrients = ({macronutrients, tooltipOpen, setTooltipOpen}) => {
    ...
    macronutrients = macronutrients?.map((macronutrient) => {
        ...
        const fullSquareCount = Math.min(10, Math.floor(macronutrient.percentOfDailyNeeds / 10));
        let filledSquares = []
        for (let i = 0; i < fullSquareCount; i++) {
            filledSquares.push(<FiSquare fill={color} className="nutrient-filled-square" id={`nutrient-filled-square-${i}`}/>)
        }
        if (fullSquareCount !== 10) {
            filledSquares.push(<FiSquare fill={color} className="nutrient-half-filled-square"/>)
        }
    ...
    {if (nutrientInfoExpanded) return (
        <>
            {macronutrients?.map((macronutrient, i) => (
                <div data-idx={i} className="single-macro-container" onMouseOver={handleMouseOver}>
                    <div className="nutrient-text">
                        <p key={i}>{macronutrient.name}</p>
                        <p>{Math.floor(macronutrient.amount)} {macronutrient.unit}</p>
                    </div>
                    ...
                    {tooltipOpen === i && (
                        <h1 className="macro-tooltip">{`${Math.floor(macronutrient.percentOfDailyNeeds)}% DV (Recommended Daily Value)`}</h1>
                    )}
                </div>
            ))}
            <div onClick={handleExpandClick}><MdOutlineExpandLess /> See Less</div>
        </>
    )};
    {if (!nutrientInfoExpanded) return (
        <>
